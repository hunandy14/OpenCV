...
def Transform_img(img):
	...
	# 藍色
	Lower_HSV = np.array([98,150,60])
	Upper_HSV = np.array([118,235,255])
...
def Target_Track(points,img):
	...
	for j in range(0,len(points)):
		...
		for i in range(0,len(Target_XY)):
			# 已經找到相同的座標，下面的不用找了
			if (Target_XY[i] != [-1]) and (Point_Distance( points[j], Target_XY[i] )) <=5:
				NewTarget_Flag=0
				break
		...
		if Target_XY[j] != [-1]:
			for i in range(0,len(points)):
				if Point_Distance(Target_XY[j],points[i]) <=Target_Distance:
					# 取得目標速度
					# print "	Target_XY",[j],Target_XY[j],"points",[i],points[i]
					Speed=Point_Distance(Target_XY[j],points[i])
					Target_Speed[j].append(Speed)
					# print "Target_Speed=",Target_Speed,"\n"
					print [j+1],"Speed=",sum(Target_Speed[j])
					if len(Target_Speed[j])>=30:
						Target_Speed[j].remove(Target_Speed[j][0])
					# 清除需放在最後面
